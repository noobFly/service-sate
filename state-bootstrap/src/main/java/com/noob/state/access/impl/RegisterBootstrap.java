...
public class RegisterBootstrap extends AbstractBootstrap {
	...
	private void registerNode(BusinessStorage storage) {
		...
		if (CommonUtil.notEmpty(providerList)) {
			...
			String rootMonitors = storage.getData(ProviderNode.ROOT); // providers节点状态
			for (Provider provider : providerList) {
				...
				serviceManager.getProviderService().initNode(providerInstancePath, rootMonitors);
				...
				if (CommonUtil.notEmpty(apiList)) {
					for (Api api : apiList) {
						...
						serviceManager.getApiService().initNode(apiInstancePath,
								MonitorUtil.join(providerAdapter.getMonitorList())); // 带上级监控状态创建实例
	 ...
	 * 
	 * @param storage
	 * @param providerInstancePath
	 *            提供者实例节点地址
	 * @param api
	 *            服务实例对象
	 * @param apiInstancePath
	 *            服务实例节点地址
	 */
	...
	private List<Monitor> generatorMonitor(BusinessStorage storage, String node) {
		return MonitorUtil.splitToMonitorList(storage.getData(node));
	}
}
