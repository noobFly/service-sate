package com.noob.state.register.impl;
 ...
 */
@Slf4j
public final class ZookeeperRegistryCenter implements ICoordinatorRegistryCenter {

	@Getter(AccessLevel.PROTECTED)
	private ZookeeperConfiguration zkConfig;

	private final Map<String, TreeCache> treeCaches = new HashMap<>();

	private final Map<String, PathChildrenCache> childrenCaches = new HashMap<>();
	...
	@Override
	public void init() {
		String serverLists = zkConfig.getServerLists();
		log.debug("zookeeper registry center init, server lists is: {}.", serverLists);
		CuratorFrameworkFactory.Builder builder = CuratorFrameworkFactory.builder().connectString(serverLists)
		...
		int sessionTimeoutMilliseconds = zkConfig.getSessionTimeoutMilliseconds();
		if (0 != sessionTimeoutMilliseconds) {
			builder.sessionTimeoutMs(sessionTimeoutMilliseconds);
		}
		int connectionTimeoutMilliseconds = zkConfig.getConnectionTimeoutMilliseconds();
		if (0 != connectionTimeoutMilliseconds) {
			builder.connectionTimeoutMs(connectionTimeoutMilliseconds);
		}
		String digest = zkConfig.getDigest();
	...
	@Override
	public void close() {
		for (Entry<String, TreeCache> each : treeCaches.entrySet()) {
			CloseableUtils.closeQuietly(each.getValue());
		}
		for (Entry<String, PathChildrenCache> each : childrenCaches.entrySet()) {
			try {
				each.getValue().close();
			} catch (IOException e) {
	...
	private TreeCache findTreeCache(final String key) {
		for (Entry<String, TreeCache> entry : treeCaches.entrySet()) {
	...
	@Override
	public void persist(final String key, final String value, final boolean update) {
		try {
			if (!isExisted(key)) {
	...
	@Override
	public void persist(final String key, final String value, final BackgroundCallback callback) {
		try {
			if (!isExisted(key)) {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).inBackground(callback)
	...
	@Override
	public void addTreeCache(final String cachePath) {
		...
		treeCaches.put(cachePath, cache);
	}

	@Override
	public void evictTreeCacheData(final String cachePath) {
		TreeCache cache = treeCaches.remove(cachePath);
		if (null != cache) {
			cache.close();
		}
	}
	...
	public TreeCache getTreeCache(final String cachePath) {
		return treeCaches.get(cachePath);
	}

	@Override
	public void addChildrenCache(final String cachePath) {
		// 设置true在事件响应时能够获取到节点内容，false不行
		PathChildrenCache cache = new PathChildrenCache(client, cachePath, true);
		try {
			cache.start();
			// CHECKSTYLE:OFF
		} catch (final Exception ex) {
	...
	@Override
	public PathChildrenCache getChildrenCache(String cachePath) {
